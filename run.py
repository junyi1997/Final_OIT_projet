...
class MyApp(object):
    ...
    def run(self):
        print("wait for run_number start......")
        if self.run_number==1:
            ...
            while True:
                ...
                self.camera.takePhoto(self.imgpath)
                    #self.camera.takePhoto(c)
                
                labels = self.classifier.get_image_labels(self.imgpath)
                print (labels)
    ...
    def ButSAVE(self,winC):
        ...
        if self.tit == "Google":
            for i in range(self.sumi_1_1,self.sumi_1):
                ...
                doc_ref = db.collection(self.tit).document(A)
        ...
        def bt_OK():
            self.run_number=1
            print("self.run_number_in_btnsave",self.run_number)
            self.clear()
            win_EXIT.destroy()
...
#######################################################################################################        
    ...
    def openFrame1(self):
        ...
        def bt_OK():
            self.run_number=0
            print("self.run_number_in_btnOK",self.run_number)
            print("save3=",self.save3)
            print("save2=",self.save2)
            print("save1=",self.save1)
            self.ButEXIT(self.win_main,self.sum1)
        ...
        if self.tit =='fb':
            ...
            self.saveK3.clear()
        self.run_number=1
        print("self.run_number_in_openFrame1",self.run_number)
...
if __name__ == "__main__":
    ...
    RN = threading.Thread(target = app.run)
    GUI=threading.Thread(target=app.__init__(win))
    GUI.start()
    RN.start()
    app.main()
    win.mainloop()